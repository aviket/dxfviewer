...
class DXFViewSettings {
    ...
    public
        ...
        register_activation_hook(DXFVIEW_FILE, array(
            $this,
            'activate'
        ));
        register_deactivation_hook(DXFVIEW_FILE, array(
            $this,
            'deactivate'
        ));
    }
    ...
    public
            function admin_init() {
        register_setting('DXFView_options', $this->option_name, array(
            $this,
            'validate'
        ));
    }
    ...
    public
            function add_page() {
        add_options_page('DXFView  Options', 'DXFView Options', 'manage_options', 'DXFView_options', array(
            $this,
            'options_dxf_page'
        ));
    }
                    ...
                    <TR>
                        ...
                        <td width=25%><b>Border Style</b>
                            <select id="bstyle" onchange="updborder()"  >
                                <option  value="solid" <?php if (explode(' ', get_option('DXFView-Settings') ['border']) [1] == 'solid'): ?> selected="selected"<?php endif;
        ?>>Solid
                                <option value="double" <?php if (explode(' ', get_option('DXFView-Settings') ['border']) [1] == 'double'): ?> selected="selected"<?php endif;
        ?>>Double
                                <option value="none" <?php if (explode(' ', get_option('DXFView-Settings') ['border']) [1] == 'none'): ?> selected="selected"<?php endif;
        ?>>None
                                <option value="dotted" <?php if (explode(' ', get_option('DXFView-Settings') ['border']) [1] == 'dotted'): ?> selected="selected"<?php endif;
        ?>>Dotted
                                <option value="dashed" <?php if (explode(' ', get_option('DXFView-Settings') ['border']) [1] == 'dashed'): ?> selected="selected"<?php endif;
        ?>>Dashed
                            </select>
                        </td>
                    </tr>
